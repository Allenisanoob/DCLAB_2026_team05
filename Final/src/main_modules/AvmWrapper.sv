...
// //   把整包資料從 o_command[255:0] 輸出，並把 o_valid 拉高「4 個 clock cycles」。
// //
// // 對外介面 (Conduit 形式，直接拉到頂層 DE2_115.sv 再接 Param_Manager)：
// //   o_command[255:0] : 累積滿的一整包 256-bit 命令
// //   o_valid          : 4-cycle pulse，告知 o_command 有效 (接收端無 ready)
// //
// // 位元拼裝順序：
...
//     output logic         o_valid    // 4-cycle valid pulse,
...
// logic [2:0]   valid_cnt_r, valid_cnt_w;  // 讓 valid 維持 4 個 avm_clk cycles
...
//     // valid 延長邏輯：
//     // 收滿封包那拍把 valid_cnt_w 設成 3。
//     // 加上當下 valid_w=1，共維持 4 個 avm_clk cycles。
//     valid_cnt_w   = valid_cnt_r;
//     if (valid_cnt_r != 3'd0) begin
//         valid_w     = 1'b1;
//         valid_cnt_w = valid_cnt_r - 3'd1;
//     end else begin
//         valid_w     = 1'b0;
//     end
...
//                     // 第 32 個 byte 收齊：這拍把整包鎖進 command_r，valid 維持 4 個 avm_clk cycles
//                     valid_w     = 1'b1;         // 在時序段對齊 command_r 更新後輸出
//                     valid_cnt_w = 3'd3;         // 加上這拍 valid_w=1，共 4 拍
...
//         valid_r       <= valid_w;    // valid_w 會在收滿後維持 4 個 avm_clk cycles
//         valid_cnt_r   <= valid_cnt_w;
//     end
// end
